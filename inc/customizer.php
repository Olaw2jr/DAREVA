<?php
/**
 * DAREVA Customizer functionality
 *
 * @package WordPress
 * @subpackage DAREVA
 * @since DAREVA 1.0
 ...
 */
function dareva_customize_register( $wp_customize ) {
	...
	$wp_customize->add_setting( 'color_scheme', array(
		'default'           => 'default',
		'sanitize_callback' => 'dareva_sanitize_color_scheme',
		'transport'         => 'postMessage',
	) );

	$wp_customize->add_control( 'color_scheme', array(
		'label'    => __( 'Base Color Scheme', 'dareva' ),
		'section'  => 'colors',
		'type'     => 'select',
		'choices'  => dareva_get_color_scheme_choices(),
		'priority' => 1,
	) );
	...
	$wp_customize->add_control( new WP_Customize_Color_Control( $wp_customize, 'sidebar_textcolor', array(
		'label'       => __( 'Header and Sidebar Text Color', 'dareva' ),
		'description' => __( 'Applied to the header on small screens and the sidebar on wide screens.', 'dareva' ),
		'section'     => 'colors',
	) ) );
	...
	$wp_customize->add_control( new WP_Customize_Color_Control( $wp_customize, 'header_background_color', array(
		'label'       => __( 'Header and Sidebar Background Color', 'dareva' ),
		'description' => __( 'Applied to the header on small screens and the sidebar on wide screens.', 'dareva' ),
		'section'     => 'colors',
	) ) );

	// Add an additional description to the header image section.
	$wp_customize->get_section( 'header_image' )->description = __( 'Applied to the header on small screens and the sidebar on wide screens.', 'dareva' );
}
add_action( 'customize_register', 'dareva_customize_register', 11 );

/**
 * Register color schemes for DAREVA.
 *
 * Can be filtered with {@see 'dareva_color_schemes'}.
 *
 ...
 */
function dareva_get_color_schemes() {
	return apply_filters( 'dareva_color_schemes', array(
		'default' => array(
			'label'  => __( 'Default', 'dareva' ),
			'colors' => array(
				'#f1f1f1',
				'#ffffff',
				'#ffffff',
...
if ( ! function_exists( 'dareva_get_color_scheme' ) ) :
/**
 * Get the current DAREVA color scheme.
 *
 * @since DAREVA 1.0
 *
 * @return array An associative array of either the current or default color scheme hex values.
 */
function dareva_get_color_scheme() {
	$color_scheme_option = get_theme_mod( 'color_scheme', 'default' );
	$color_schemes       = dareva_get_color_schemes();
...
endif; // dareva_get_color_scheme

if ( ! function_exists( 'dareva_get_color_scheme_choices' ) ) :
/**
 * Returns an array of color scheme choices registered for DAREVA.
 *
 * @since DAREVA 1.0
 *
 * @return array Array of color schemes.
 */
function dareva_get_color_scheme_choices() {
	$color_schemes                = dareva_get_color_schemes();
...
endif; // dareva_get_color_scheme_choices

if ( ! function_exists( 'dareva_sanitize_color_scheme' ) ) :
/**
 ...
 */
function dareva_sanitize_color_scheme( $value ) {
	$color_schemes = dareva_get_color_scheme_choices();
...
endif; // dareva_sanitize_color_scheme
 ...
 */
function dareva_color_scheme_css() {
	$color_scheme_option = get_theme_mod( 'color_scheme', 'default' );
	...
	$color_textcolor_rgb         = dareva_hex2rgb( $color_scheme[3] );
	$color_sidebar_textcolor_rgb = dareva_hex2rgb( $color_scheme[4] );
	$colors = array(
	...
	$color_scheme_css = dareva_get_color_scheme_css( $colors );

	wp_add_inline_style( 'dareva-style', $color_scheme_css );
}
add_action( 'wp_enqueue_scripts', 'dareva_color_scheme_css' );
 ...
 * @since DAREVA 1.0
 */
function dareva_customize_control_js() {
	...
	wp_localize_script( 'color-scheme-control', 'colorScheme', dareva_get_color_schemes() );
}
add_action( 'customize_controls_enqueue_scripts', 'dareva_customize_control_js' );
 ...
 * @since DAREVA 1.0
 */
function dareva_customize_preview_js() {
	wp_enqueue_script( 'dareva-customize-preview', get_template_directory_uri() . '/js/customize-preview.js', array( 'customize-preview' ), '20141216', true );
}
add_action( 'customize_preview_init', 'dareva_customize_preview_js' );
 ...
 */
function dareva_get_color_scheme_css( $colors ) {
 ...
 * @since DAREVA 1.0
 */
function dareva_color_scheme_css_template() {
	$colors = array(
	...
	?>
	<script type="text/html" id="tmpl-dareva-color-scheme">
		<?php echo dareva_get_color_scheme_css( $colors ); ?>
	</script>
	<?php
}
add_action( 'customize_controls_print_footer_scripts', 'dareva_color_scheme_css_template' );
